...
const Header = () => {
  return (
    <div className="header">
      <h1>
        <span>Service Request</span> &#47; Active{" "}
      </h1>
    </div>
  );
};
...
const ServiceRequestsActive = () => {
  const [requests, setRequests] = useState({});
  const [loading, setLoading] = useState(true);
  const data = "";
  const apiConfig = {
    method: "post",
    url: "https://www.naataconnection.com/api/serviceRequest/all/active",
    headers: {},
    data: data,
  };
...
export default ServiceRequestsActive;
