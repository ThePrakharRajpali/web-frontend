...
const Header = () => {
  return (
    <div className="header">
      <h1>
        <span>Service Request</span> &#47; Pending{" "}
      </h1>
    </div>
  );
};
...
const ServiceRequestsPending = () => {
  const [requests, setRequests] = useState({});
  const [loading, setLoading] = useState(true);
  const data = "";
  const apiConfig = {
    method: "post",
    url: "https://www.naataconnection.com/api/serviceRequest/all/pending",
    headers: {},
    data: data,
  };
...
export default ServiceRequestsPending;
