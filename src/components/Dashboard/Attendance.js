...
class Attendance extends React.Component {
  ...
  handleClick(event) {
    ...
    updateTable(
      ...
      this.state.endDate,
      this.state.userCodes.concat(userCode)
    );
  }
  ...
  render() {
            ...
            return (
              <tr className={index % 2 ? "AttendanceRow1" : "AttendanceRow2"}>
                <td className="AttendanceCell">
                  {fullName(item)}
                  <button
                    className="AttendanceUserDelete"
                    onClick={() => this.removeUser(item)}
                    value={item}
                    type="button"
                  >
                    <IconContext.Provider
                      value={{ color: "#E5584F", size: "1vw" }}
                    >
      ...
      attendaceTableComponent = (
        <>
          {responses.map((item, index) => {
            const NA1 = Math.max(
              diffNumDays(startDate, item["dateOfJoining"]),
              0
            );
            const NA2 = Math.max(diffNumDays2(endDate), 0);
            const mid = Math.max(
              diffNumDays(startDate, endDate) - NA1 - NA2 + 1,
              0
            );
            ...
            return (
              <tr className={index % 2 ? "AttendanceRow1" : "AttendanceRow2"}>
                <td className="AttendanceCell">
                  ...
                  <button
                    className="AttendanceUserDelete"
                    onClick={() => this.removeUser(item["userCode"])}
                    value={item["userCode"]}
                    type="button"
                  >
                    <IconContext.Provider
                ...
                {Array.from(Array(NA1), (e, i) => {
                  return <td className="AttendanceCell">--</td>;
                })}
                {item.data.map((itemInner, indexInner) => {
                  ...
                  if (itemInner.attendance_status === 1) {
                    return (
                      <td className="AttendanceCell">
                        ...
                        <IconContext.Provider
                          ...
                          value={{ color: "#F3752B", size: "0.75vw" }}
                        >
                          <AiIcons.AiOutlineClockCircle
                            style={{ cursor: "pointer" }}
                            onClick={() =>
                              showTime(fullNameOfUser, itemInner.startTime)
                            }
                          />
                          &nbsp;
                          <a
                    ...
                    return <td className="AttendanceCell">A</td>;
                  }
                })}
    ...
    if (!loaded) {
      component = (
        ...
        >
          <center>
            <h1 style={{ color: "#0f1185", fontSize: "7vw", margin: "auto" }}>
              Loading Page !!!
            </h1>
          </center>
        </div>
        ...
        <div>
          <div>
            ...
            <div className="SearchBarIcon">
              <IconContext.Provider
                className="SearchBarIcon"
                value={{ color: "#ffffff", size: "1.33vw" }}
              >
          ...
          <div className="DateRangeRow">
            ...
            <button
              ...
              onClick={this.handleClickDateChange}
            ></button>
          </div>

          <div className="clearAttendanceRow">
            <button
              className="clearAttendanceButton"
            ...
            <table className="AttendanceTable">
              <tr className="AttendanceRow1">
                ...
                {this.state.dates.map((item) => {
                  return <td className="AttendanceCell">{item}</td>;
                })}
              </tr>
